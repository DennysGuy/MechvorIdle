...
func show_data_labels() -> void:
	if selected_drone is MiningDrone:
		sell_button.text = "Sell for " + str(GameManager.drones_cost/2)
	elif selected_drone is PlatinumMiningDrone:
		sell_button.text = "Sell for " + str(GameManager.platinum_drone_cost/2)
	sell_button.show()
	icon.show()
...
func _on_sell_button_button_down():
	if selected_drone is MiningDrone:
		GameManager.platinum_count += round(GameManager.drones_cost/2)
	elif selected_drone is PlatinumMiningDrone:
		GameManager.platinum_count += round(GameManager.platinum_drone_cost/2)
