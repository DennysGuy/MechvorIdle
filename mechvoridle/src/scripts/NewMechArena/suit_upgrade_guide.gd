class_name SuitUpgradeGuide extends Node3D

var crate_to_look_at : UpgradeCrate
var crates : Array[UpgradeCrate]

@onready var suit_upgrade_guide: Sprite3D = $SuitUpgradeGuide

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	SignalBus.init_suit_upgrade_guide.connect(enable_guide)
	SignalBus.disable_suit_upgrade_guide.connect(disable_guide)
	SignalBus.check_once_for_suit_crates.connect(check_for_crates)
	suit_upgrade_guide.hide()
...
func check_for_crates(crate : UpgradeCrate) -> void:
	remove_crate(crate)
