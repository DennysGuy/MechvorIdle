...
@onready var crit_damage_perc : Label = $CritDamagePerc
...
@onready var crit_damage_perc_2 : Label = $CritDamagePerc2
...
func update_total_mobility(component : MechLegs) -> void:
	var added_dodge_chance : float = GameManager.BASE_DODGE_CHANCE * component.dodge_chance_modifier
	mobility_amount.text = str(GameManager.BASE_DODGE_CHANCE + added_dodge_chance) +"%" 
...
func update_weapon_1_charge_speed_on_weapon_purcahse(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Torso"]:
		...
		var torso_charge_speed_bonus : float = owned_torso.charge_speed
		var added_charge_speed_bonus : float = charge_speed * torso_charge_speed_bonus * 100
		charge_speed_amount.text = calculate_stat(component, owned_torso, charge_speed, added_charge_speed_bonus)
...
func update_weapon_charge_speed_on_torso_purchase(component : MechTorso) -> void:
	...
	if GameManager.owned_mech_components["LeftWeapon"]:
		...
		charge_speed_amount.text = calculate_stat(left_weapon, component, charge_speed, added_charge_speed_bonus)
	...
	if GameManager.owned_mech_components["RightWeapon"]:
		...
		var charge_speed : float = right_weapon.charge_speed*100
		var added_charge_speed_bonus : float = charge_speed * torso_charge_speed_bonus * 100
		charge_speed_amount.text = calculate_stat(right_weapon, component, charge_speed, added_charge_speed_bonus)
...
func update_weapon_1_crit_chance_on_weapon_purchase(component : MechWeapon) -> void:
	...
	if GameManager.owned_mech_components["Head"]:
		...
		crit_chance_perc.text = calculate_stat(component, owned_head, crit_chance, added_crit_chance_bonus)+ "%"
	else:
...
func update_weapon_crit_chance_on_head_purchase(component : MechHead) -> void:
	...
	if GameManager.owned_mech_components["LeftWeapon"]:
		...
		crit_chance_perc.text = calculate_stat(left_weapon, component, weapon_crit_chance, added_crit_chance_bonus) + "%"

	if GameManager.owned_mech_components["RightWeapon"]:
		var right_weapon : MechWeapon = GameManager.owned_mech_components["RightWeapon"]
		var weapon_crit_chance : float = roundf(right_weapon.crit_chance * 100)
		var added_crit_chance_bonus : float = roundf(weapon_crit_chance * crit_chance_bonus)
		crit_chance_perc_2.text = calculate_stat(right_weapon, component, weapon_crit_chance, added_crit_chance_bonus) + "%"
...
func update_weapon_1_accuracy_on_weapon_purchase(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Head"]:
		...
		accuracy_rate.text = calculate_stat(component, owned_head, hit_chance, added_bonus) +"%"
...
func update_weapon_accuracy_on_head_purchase(component : MechHead) -> void:
	...
	if GameManager.owned_mech_components["LeftWeapon"]:
		...
		var added_bonus : float = roundf(hit_chance * hit_chance_bonus)
		accuracy_rate.text = calculate_stat(left_weapon, component, hit_chance_bonus, added_bonus) + "%"

	if GameManager.owned_mech_components["RightWeapon"]:
		var right_weapon : MechWeapon = GameManager.owned_mech_components["RightWeapon"]
		var hit_chance_bonus : float = roundf(right_weapon.accuracy * 100)
		var added_bonus : float = roundf(hit_chance * hit_chance_bonus)
		accuracy_rate_2.text = calculate_stat(right_weapon, component, hit_chance_bonus, added_bonus)+ "%"
...
func update_weapon_2_charge_speed_on_weapon_purcahse(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Torso"]:
		...
		var added_charge_speed_bonus : float = charge_speed * torso_charge_speed_bonus
		charge_speed_amount_2.text = calculate_stat(component, owned_torso, charge_speed, added_charge_speed_bonus)
...
func update_weapon_2_crit_chance_on_weapon_purchase(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Head"]:
		...
		var owned_head : MechHead = GameManager.owned_mech_components["Head"]
		var head_crit_chance_bonus : float = roundf(owned_head.crit_chance*100)
		crit_chance_perc_2.text = calculate_stat(component, owned_head, crit_chance, head_crit_chance_bonus) + "%"
...
func update_weapon_2_accuracy_on_weapon_purchase(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Head"]:
		...
		var owned_head : MechHead = GameManager.owned_mech_components["Head"]
		var hit_chance_bonus : float = roundf(owned_head.accuracy_bonus * 100)
		accuracy_rate_2.text = calculate_stat(component, owned_head, hit_chance, hit_chance_bonus) + "%"
...
func update_weapon_2_stun_chance_on_weapon_purchase(component : MechWeapon) -> void:
	...
	if !GameManager.owned_mech_components["Legs"]:
		...
		var owned_legs : MechLegs = GameManager.owned_mech_components["Legs"]
		var stun_chance_bonus : float = roundf(owned_legs.stun_chance*100)
		stun_chance_w_1_perc_2.text = str(stun_chance + stun_chance_bonus) + "%" + " ("+str(stun_chance)+" + "+str(stun_chance_bonus)+")"

func update_weapon_2_crit_damage_on_weapon_purchase(component : MechWeapon) -> void:
	pass

func calculate_stat(weapon : MechWeapon, component : MechComponent, base_stat : float, bonus_stat : float) -> String:
	match (component.get_weight_class()):
		"HEAVY":
			if weapon.component_type == weapon.COMPONENT_TYPE.RANGED:
				return str(roundf(base_stat + bonus_stat))+" ("+str(base_stat)+" + "+str(bonus_stat)+")"
			else:
				return str(roundf(base_stat - bonus_stat))+" ("+str(base_stat)+" - "+str(bonus_stat)+")"
		"REGULAR":
			return str(roundf(base_stat + bonus_stat)) +" ("+str(base_stat)+" + "+str(bonus_stat)+")"
