class_name SubMachineGunFireLeft extends WeaponState
...
func exit() -> void:
	...
	SignalBus.issue_weapon_attack.emit(POSITION.LEFT)
...
func fire_smg_muzzle_flare_left() -> void:
	var muzzle_flare : WeaponMuzzleFlare =  preload("uid://bqgicaqmm0vsq").instantiate()
	muzzle_flare.global_position = parent.rifle_2_spout.global_position
